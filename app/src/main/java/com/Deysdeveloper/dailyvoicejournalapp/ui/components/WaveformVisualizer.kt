...
fun WaveformVisualizerLive(
    ...
    LaunchedEffect(amplitude) {
        // More efficient approach: use copy instead of creating new list
        amplitudeHistory = if (amplitudeHistory.size < 40) {
            amplitudeHistory + amplitude
        } else {
            amplitudeHistory.drop(1) + amplitude
        }
    }
